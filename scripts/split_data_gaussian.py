...
def read_multipoles(
    ...
    data = np.load(
        Path(f"../data/different_hods/full_ap/clustering/ds/gaussian/")
        / f"ds_cross_xi_smu_zsplit_Rs20_c{cosmo_idx:03}_ph000.npy",
        allow_pickle=True,
    ).item()
...
def read_params(cosmo_idx):
    params = pd.read_csv(
        Path(f"../data/different_hods/full_ap/cosmologies/")
        / f"AbacusSummit_c{cosmo_idx}_hod1000.csv"
    )
...
if __name__ == "__main__":
    ...
    np.save("../data/different_hods/training/ds/gaussian/train_params.npy", params[train_idx].reshape(-1, params.shape[-1]))
    np.save("../data/different_hods/training/ds/gaussian/test_params.npy", params[test_idx].reshape(-1, params.shape[-1]))
    np.save(
        "../data/different_hods/training/ds/gaussian/val_params.npy",
        params[val_idx].reshape(-1, params.shape[-1]),
    )
    print('params train', np.shape(params[train_idx].reshape(-1, params.shape[-1])))
    print('params test', np.shape(params[test_idx].reshape(-1, params.shape[-1])))
    print('params val', np.shape(params[val_idx].reshape(-1, params.shape[-1])))
    # multipoles = []
    # for idx in cosmo_idx:
    #     combined_multi = []
    ...
    for quintile in quintiles:
        ...
        multipoles = np.array(multipoles)
        np.save(
            f"../data/different_hods/training/ds/gaussian/s.npy",
            s,
        )
        s = np.array(list(s) + list(s))
        np.save(
            f"../data/different_hods/training/ds/gaussian/train_ds{quintile}.npy",
            multipoles[train_idx].reshape(-1, multipoles.shape[-1]),
        )
        print('train', np.shape(multipoles[train_idx].reshape(-1, multipoles.shape[-1])))
        np.save(
            f"../data/different_hods/training/ds/gaussian/test_ds{quintile}.npy",
            multipoles[test_idx].reshape(-1, multipoles.shape[-1]),
        )
        print('test', np.shape(multipoles[test_idx].reshape(-1, multipoles.shape[-1])))
        np.save(
            f"../data/different_hods/training/ds/gaussian/val_ds{quintile}.npy",
            multipoles[val_idx].reshape(-1, multipoles.shape[-1]),
        )
        print('val', np.shape(multipoles[val_idx].reshape(-1, multipoles.shape[-1])))
        store_summaries(
            s,
            params[train_idx].reshape(-1, params.shape[-1]),
            multipoles[train_idx].reshape(-1, multipoles.shape[-1]),
            f"../data/different_hods/training/ds/gaussian/train_ds{quintile}_summary.json",
        )
