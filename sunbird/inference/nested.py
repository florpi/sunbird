...
class Nested(Inference):
    ...
    def __call__(self,log_dir, slice_steps, num_live_points):
        ...
        sampler.run(dlogz=0.5 + 0.1 * len(self.param_names),
    ...
    def get_results(self,):
        chain = np.loadtxt(self.inference_config["outputfiles_basename"] + f"smin{self.s_min:.2f}.txt")
        df = pd.DataFrame({"log_likelihood": chain[:, 1], "weights": chain[:, 0],})
