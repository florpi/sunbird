...
samples_list = []
samples_planck = loadMCSamples('/pscratch/sd/e/epaillas/planck/base_nrun/plikHM_TTTEEE_lowl_lowE/base_nrun_plikHM_TTTEEE_lowl_lowE', settings={'ignore_rows': 0.3})
...
g.plot_1d(samples_list, 'nrun', filled=True,)
xmin, xmax = g.fig.axes[0].get_xlim()
g.fig.axes[0].fill_between([xmin, priors['nrun'][0]], 0.0, 1.0, color='grey', alpha=0.1)
g.fig.axes[0].fill_between([priors['nrun'][1], xmax], 0.0, 1.0, color='grey', alpha=0.1)
plt.ylabel('Posterior density', fontsize=23)
plt.xlabel(r'${\rm d}n_s/{\rm d}\ln k$', fontsize=23)
plt.ylim(0.0, 1.65)
# plt.xlim(-1.2, -0.8)
...
plt.tight_layout()
plt.savefig('fig/pdf/cosmo_inference_cmass_nrun.pdf', bbox_inches='tight')