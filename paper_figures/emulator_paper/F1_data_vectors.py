...
if __name__ == "__main__":
    ...
    args.add_argument(
        ...
        type=str,
        nargs="*",
        default=["density_split_cross", "density_split_auto", "tpcf"],
    )
    args.add_argument(
        "--ell",
        type=int,
    ...
    for statistic in args.statistic:
        for ell in args.ell:
            ...
            for ds in quantiles:
                ...
                datavector = abacus.get_observation(
                    cosmology=cosmology,
                    hod_idx=hod_idx,
                )
                ...
                cov_data = cov.get_covariance_data(volume_scaling=64.0)
            ...
            plt.savefig(
                f"figures/png/F1_multipoles_c0_{statistic}_ell{ell}.png", dpi=300
            )
            plt.savefig(
                f"figures/pdf/F1_multipoles_c0_{statistic}_ell{ell}.pdf", dpi=300
            )
            plt.show()
