...
def plot_gaussianity(summaries, ax, colors=['#4165c0','#e770a2']):
    ...
    bins = ax.hist(
        xi2_data, bins=50, density=True, label="Data", alpha=0.5, edgecolor=None,
        color=colors[0],
    )
    ax.hist(
        xi2_random,
...
if __name__ == "__main__":
    ...
    ax[1].set_ylabel(r"$\mathrm{PDF}$", )
    ax[2].set_ylabel(r"$\mathrm{PDF}$", )
    ax[0].set_title(r"$\mathrm{2PCF}$", )
    ...
    plt.tight_layout()
    plt.savefig("figures/png/A1_gaussianity_likelihood.png", dpi=300)
    plt.savefig("figures/pdf/A1_gaussianity_likelihood.pdf")