...
def get_emulator_and_truth(
    split,
    select_filters= {'quintiles':[0,1,3,4], 'multipoles': [0,1],},
):
...
def get_emulator_and_truth_for_cosmo(
    ...
    for cosmo in cosmo_idx:
        parameters.append(abacus.get_all_parameters(cosmology=cosmo,))
    ...
    parameters = torch.tensor(parameters, dtype=torch.float32).reshape(-1, parameters.shape[-1])
    ...
    emulated_density_split = emulator.forward(
        parameters,
        select_filters=select_filters,
    ).detach().numpy()
