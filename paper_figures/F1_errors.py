...
fig, ax = plt.subplots(
    nrows=4,figsize=(9, 6), sharex=True, sharey=True 
)
...
for q in range(4):
    ax[0].fill_between(
        ...
        (avg_error[0,:,0].reshape(-1) - std_error[0,:,0].reshape(-1))[q*len(s):(q+1)*len(s)], 
        (avg_error[0,:,0].reshape(-1) + std_error[0,:,0].reshape(-1))[q*len(s):(q+1)*len(s)], 
        alpha=0.5,
        color=ds_colors[q],
    ...
    ax[1].fill_between(
        x_range[q*len(s):(q+1)*len(s)],
        (avg_error[0,:,1].reshape(-1) - std_error[0,:,1].reshape(-1))[q*len(s):(q+1)*len(s)], 
        (avg_error[0,:,1].reshape(-1) + std_error[0,:,1].reshape(-1))[q*len(s):(q+1)*len(s)], 
        alpha=0.5,
        color=ds_colors[q],
    )
    ax[2].fill_between(
        x_range[q*len(s):(q+1)*len(s)],
        (avg_error[1,:,0].reshape(-1) - std_error[1,:,0].reshape(-1))[q*len(s):(q+1)*len(s)], 
        (avg_error[1,:,0].reshape(-1) + std_error[1,:,0].reshape(-1))[q*len(s):(q+1)*len(s)], 
        alpha=0.5,
        color=ds_colors[q],
    )
    ax[3].fill_between(
        x_range[q*len(s):(q+1)*len(s)],
        (avg_error[1,:,1].reshape(-1) - std_error[1,:,1].reshape(-1))[q*len(s):(q+1)*len(s)], 
        (avg_error[1,:,1].reshape(-1) + std_error[1,:,1].reshape(-1))[q*len(s):(q+1)*len(s)], 
        alpha=0.5,
        color=ds_colors[q],
    )
...
_ = ax[-1].set_xticks(
    current_labels[:-1],
    [all_s[int(c)]-0.5 for c in current_labels[:-1]] 
)
